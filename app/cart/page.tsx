...
export default function CartPage() {
  ...
  // Safely extract totals calculated by backend
  const subtotal = rawCartSubtotal;
  const shippingCost = totals?.shipping ?? 0;
  const tax = totals?.tax ?? 0;
  const grandTotal =
    totals?.grandTotal ?? subtotal + shippingCost + tax;
        ...
        <Card className="lg:sticky lg:top-24 h-fit">
          ...
          <CardContent className="space-y-4">
            ...
            <Link href="/checkout">
              <Button
                size="lg"
                className="w-full text-base font-bold mt-4"
              >
                Proceed to Checkout
              </Button>
            </Link>
            ...
            <p className="text-xs text-center text-foreground/60 pt-2">
              Discount calculated and final totals confirmed at
              checkout.
            </p>
          </CardContent>
        </Card>
